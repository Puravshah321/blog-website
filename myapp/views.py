...
def blog_view(request):
    ...
    if selected_categories:
        posts = Post.objects.filter(category__in=selected_categories).distinct()
    else:
        posts = Post.objects.all()

    context = {
        'recent_posts': posts,
        'categories': categories,  # Pass categories for dynamic sidebar rendering
...
from .models import Post  # Assuming Post is your model for posts
from django.shortcuts import render

def search(request):
    query = request.GET.get('query', '')
    search_type = request.GET.get('search_type', 'users')  # Default to 'users'

    if search_type == 'users':
        search_results = User.objects.filter(username__icontains=query) if query else None
    elif search_type == 'posts':
        search_results = Post.objects.filter(postname__icontains=query) if query else None
    else:
        search_results = None
